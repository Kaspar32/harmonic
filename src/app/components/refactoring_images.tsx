...
export default function Refactoring_Images() {
      ...
      setImagesContainer((prev) =>
        prev.map((item, index) => ({
          ...
          previewUrl: user.profile_pics?.[index]
            ? `/images/${user.profile_pics[index]}?targetuuid=${user.uuid}`
            : item.previewUrl,
        })),
      );
    }
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over?.id) {
      ...
      // Prevent moving to empty containers
      if (!imagesContainer[newIndex].previewUrl) return;
          ...
          {imagesContainer.map((img, index) => (
            ...
            >
              ...
              >
                ...
                <button onClick={savePictures} className="absolute bottom-2 right-2">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    fill="currentColor"
                    className="size-6 text-green-400 hover:text-green-800"
                  >
                    <path
                      fillRule="evenodd"
                      d="M6.32 2.577a49.255 49.255 0 0 1 11.36 0c1.497.174 2.57 1.46 2.57 2.93V21a.75.75 0 1 1-1.085.67L12 18.089l-7.165 3.583A.75.75 0 0 1 3.75 21V5.507c0-1.47 1.073-2.756 2.57-2.93Z"
                      clipRule="evenodd"
                    />
                  </svg>
                </button>
                ...
                {imagesData?.[index] && (
                  <div onClick={() => deleteImage(`${imagesData[index]}`)} className="absolute top-2 right-2">
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      viewBox="0 0 24 24"
                      fill="currentColor"
                      className="size-6 text-red-400 hover:text-red-800"
                    >
                      <path
                        fillRule="evenodd"
