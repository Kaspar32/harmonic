...
export default function Profil_Edit() {
  ...
  const [showGrösse, setGrösse] = useState(false);
  ...
  const [intressen, setinteressen] = useState(Interests);
  const [genres, setGenresState] = useState(Genres);
  const [ichSucheState, setIchSucheState] = useState(IchSucheData);
  ...
  const [dataImages, setDataImages] = useState<Pics[]>([
    {
      ...
      userUuid: null,
      position: 0,
      findIndex: function (arg0: (item: any) => boolean): unknown {
        throw new Error("Function not implemented.");
      },
      map: function (arg0: (img: any) => any): Pics {
        throw new Error("Function not implemented.");
      },
    },
    {
  ...
  async function search_artist() {
    ...
    setArtists(
      Artists.artists.items.map((t: any) => ({
        ...t,
        isSelected: false,
      }))
    );
      ...
      <div className="w-[280] mb-4">
        ...
        {showName && (
          <PopUp onClose={() => setName(false)}>
            <h2 className="text-xl font-bold mb-4 text-gray-400 dark:text-gray-400">Name</h2>
            <textarea
              value={userData.name}
              onChange={(e) =>
        ...
        {showAlter && (
          <PopUp onClose={() => setAlter(false)}>
            <h2 className="text-xl font-bold mb-4  text-gray-400 dark:text-gray-400 ">Alter</h2>
            ...
            <input
              ...
              onChange={(e) =>
                setUserData((prev) => ({ ...prev, alter: e.target.value }))
              }
            ...
            <div className="flex gap-4 mt-4">
              <button
                ...
                onClick={async () => {
                  setAlter(false);
                  await updateUser({ alter: userData.alter });
                }}
              >
                Speichern
              </button>
        ...
        {showGeschlecht && (
          <PopUp onClose={() => setGeschlecht(false)}>
            <h2 className="text-xl text-gray-400 dark:text-gray-400 font-bold mb-4">Geschlecht</h2>

            <select
              value={userData.geschlecht}
              onChange={(e) =>
                setUserData((prev) => ({ ...prev, geschlecht: e.target.value }))
              }
            ...
            <div className="flex gap-4 mt-4">
              <button
                ...
                onClick={async () => {
                  setGeschlecht(false);
                  await updateUser({ geschlecht: userData.geschlecht });
                }}
              >
                Speichern
              </button>
        ...
        {/* Grösse */}
        <h3
          onClick={() => setGrösse(true)}
          className="ml-4  mt-4 text-gray-300 text-center font-semibold border-2  text-2xl hover:bg-white rounded-2xl shadow-lg "
        >
          Grösse: {userData.grösse} cm
        </h3>
        {showGrösse && (
          <PopUp onClose={() => setGrösse(false)}>
            <label
              htmlFor="groesse"
              className="block mb-2 text-lg text-gray-400 dark:text-gray-400"
            >
              Größe: {userData.grösse} cm
            </label>
            <input
              type="range"
              id="groesse"
              ...
              min="140"
              max="210"
              value={userData.grösse}
              onChange={(e) =>
                setUserData((prev) => ({ ...prev, grösse: e.target.value }))
              }
            ...
            <button
              ...
              onClick={async () => {
                setGrösse(false);
                await updateUser({ grösse: userData.grösse });
              }}
            >
              Speichern
            </button>
        ...
        {showAusbildung && (
          <PopUp onClose={() => setAusbildung(false)}>
            <h2 className="text-xl text-gray-400 dark:text-gray-400 font-bold mb-4">Ausbildung</h2>

            <select
              value={userData.ausbildung}
              onChange={(e) =>
                setUserData((prev) => ({ ...prev, ausbildung: e.target.value }))
              }
              className="relative left-20 w-[200] text-center text-black dark:text-black appearance-none focus:outline-none"
            >
              <option className="bg-gray-200">Auswählen</option>
            ...
            <div className="flex gap-4 mt-4">
              <button
                ...
                onClick={async () => {
                  setAusbildung(false);
                  await updateUser({ ausbildung: userData.ausbildung });
                }}
              >
                Speichern
              </button>
        ...
        <h3
          ...
          className="ml-4  mt-4  text-gray-300 text-center font-semibold border-2  text-2xl hover:bg-white rounded-2xl shadow-lg  "
        >
          Intressen (bitte klicken)
        </h3>
        {showInteressen && (
          <PopUp onClose={() => setInteressen(false)}>
            <h2 className="text-xl text-gray-400 dark:text-gray-400 font-bold mb-4">Interessen</h2>
              ...
              {intressen.map((interest, index) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    const updated = userData.intressen?.includes(interest.name)
                      ? userData.intressen.filter(
                          (n: string) => n !== interest.name
                        )
                      : [...(userData.intressen || []), interest.name];

                    setUserData((prev) => ({ ...prev, intressen: updated }));
                  }}
                >
                  {interest.name}
                </button>
        ...
        {showSuche && (
          <PopUp onClose={() => setSuche(false)}>
            <h2 className="text-xl text-gray-400 dark:text-gray-400 font-bold mb-4">Ich suche</h2>
      ...
      <div className="w-[280] flex-1">
        ...
        {showGenres && (
          <PopUp onClose={() => setGenres(false)}>
            <h2 className="text-xl text-gray-400 dark:text-gray-400 font-bold mb-4">Genres</h2>
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    const updated = userData.genres?.includes(genre.name)
                      ? userData.genres.filter((n: string) => n !== genre.name)
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="w-full max-w-sm">
                <input
                  className="w-full border border-slate-300 rounded-md px-3 py-2 text-sm focus: outline-yellow-300 text-slate-700 placeholder:text-slate-400"
                  placeholder="Type here..."
                  onChange={(event) => setSerachInput_Artist(event.target.value)}
                />
              </div>
