...
export default function LikesTest() {
  ...
  return (
    <div className="relative">
      ...
      {/* Panel: Likes You */}
      <div
        className={`absolute md:h-[500px] w-full overflow-y-auto h-[570px] bg-yellow-100 rounded-2xl rounded-tl-none shadow-xl ${
          toggleLikesYou ? "opacity-100 block" : "opacity-0 hidden"
        }`}
      >
        <div className="flex flex-wrap gap-4 m-20 justify-center">
          {loading && (
              ...
              {images_youlikes.length > 0 ? (
                images_youlikes.map((img, index) =>
                  img?.imageBase64_blurred ? (
                    <div
                      key={index}
                      className="border-2 rounded-2xl p-2 border-yellow-400 bg-yellow-100"
                    >
                      <Image
                        src={img.imageBase64_blurred as string}
                        alt={`Bild ${index + 1}`}
                        height={100}
                        width={100}
                ...
                images_likes.map((img, index) =>
                  img?.imageBase64 ? (
                    ...
                    >
                      ...
                      />
                      <div className="text-yellow-400">
                        {users_likes[index].name},{" "}
                        {users_likes[index].alter}
                      </div>
                    </div>
                  ) : null
                )
